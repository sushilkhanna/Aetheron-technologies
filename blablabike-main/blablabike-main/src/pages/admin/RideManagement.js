...
// ── Map: smooth pan when driver moves ────────────────────────────────────────
      ...
      <div className="relative bg-gray-900 border border-orange-500/30 rounded-2xl max-w-3xl w-full overflow-hidden shadow-2xl shadow-orange-500/10">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-orange-500/20 bg-gray-900">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-white font-semibold">Live Tracking — Ride #{ride.rideId}</h3>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-gray-800 border border-orange-500/30 rounded-2xl max-w-xl w-full max-h-[90vh] overflow-y-auto p-6 shadow-2xl shadow-orange-500/10">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-3">
              <h2 className="text-xl font-bold text-white">Ride #{ride.rideId}</h2>
              <StatusBadge state={ride.state} />
            </div>
...
const RideManagement = () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (loc) {
          ...
          locationRef.current = {
            ...loc,
            currentLat: parseFloat(loc.currentLat),
            currentLng: parseFloat(loc.currentLng),
            fromLat:    parseFloat(loc.fromLat),
            fromLng:    parseFloat(loc.fromLng),
  ...
  useEffect(() => {
    if (mapRide) {
      ...
      fetchLocation(mapRide.rideId);
      mapPollRef.current = setInterval(() => fetchLocation(mapRide.rideId), MAP_POLL_MS);
    } else {
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-orange-500/10">
                    {rides.map((ride, idx) => {
                      const canTrack = ['LIVE','STARTED','VERIFIED'].includes(ride.state);
                      return (
                        <tr key={ride.rideId || idx} className="hover:bg-gray-700/30 transition-colors">
                          <td className="px-4 py-3">
                            <span className="text-sm font-mono text-white/80">#{ride.rideId}</span>
                          </td>
                          <td className="px-4 py-3">
